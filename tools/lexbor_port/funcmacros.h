#define _lxb_html_tokenizer_state_token_done_m(tkz,v_end) if (!(tkz->opt & LXB_HTML_TOKENIZER_OPT_ATTR_KEEP_DUPLICATE)) { lxb_html_tokenizer_attr_last_duplicate(tkz); } if (tkz->token->type & LXB_HTML_TOKEN_TYPE_CLOSE) { lxb_html_tokenizer_validate_close_tag(tkz); } tkz->token = tkz->callback_token_done(tkz, tkz->token, tkz->callback_token_ctx); if (tkz->token == NULL) { if (tkz->status == LXB_STATUS_OK) { tkz->status = LXB_STATUS_ERROR; } return v_end; }
...
#define lxb_html_tokenizer_state_token_text_done_m(tkz,v_end) do { if (tkz->token->text_start != tkz->token->text_end) { _lxb_html_tokenizer_state_token_done_m(tkz, v_end); } lxb_html_token_clean(tkz->token); tkz->pos = tkz->start; } while (0)
...
#define lxb_html_tokenizer_state_token_done_wo_check_m(tkz,v_end) lxb_html_tokenizer_state_token_done_m(tkz, v_end)
...
#define lxb_html_tokenizer_state_token_emit_text_not_empty_m(tkz,v_end) do { if (tkz->start != tkz->pos) { tkz->token->tag_id = LXB_TAG__TEXT; lxb_html_tokenizer_state_set_text(tkz); _lxb_html_tokenizer_state_token_done_m(tkz, v_end) lxb_html_token_clean(tkz->token); } } while (0)
...
#define lxb_html_tokenizer_state_token_attr_add_m(tkz,attr,v_return) do { if (!(tkz->opt & LXB_HTML_TOKENIZER_OPT_ATTR_KEEP_DUPLICATE)) { lxb_html_tokenizer_attr_last_duplicate(tkz); } attr = lxb_html_token_attr_append(tkz->token, tkz->dobj_token_attr); if (attr == NULL) { tkz->status = LXB_STATUS_ERROR_MEMORY_ALLOCATION; return v_return; } } while (0)
#define lxb_html_tokenizer_state_token_done_m(tkz,v_end) do { _lxb_html_tokenizer_state_token_done_m(tkz, v_end) lxb_html_token_clean(tkz->token); tkz->pos = tkz->start; } while (0)
