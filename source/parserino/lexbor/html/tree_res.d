module parserino.lexbor.html.tree_res;
...
struct lxb_html_tree_res_attr_adjust_t {
    const(char)* from;
    const(char)* to;
    size_t len;
}

struct lxb_html_tree_res_attr_adjust_foreign_t {
    const(char)* name;
    const(char)* prefix;
...
const(lxb_html_tree_res_attr_adjust_t)[59] lxb_html_tree_res_attr_adjust_svg_map = [
...
const(lxb_html_tree_res_attr_adjust_foreign_t)[11] lxb_html_tree_res_attr_adjust_foreign_map = [
