module parserino.lexbor.html.tree.insertion_mode.in_table_text;

// D port of lexbor (https://github.com/lexbor/lexbor), Apache-2.0.
// Original author: Alexander Borisov <borisov@lexbor.com>

import parserino.lexbor.html.tree.insertion_mode;
import parserino.lexbor.html.token;
...
bool lxb_html_tree_insertion_mode_in_table_text(lxb_html_tree_t* tree, lxb_html_token_t* token)
{
    ...
    if (token.tag_id == LXB_TAG__TEXT) {
        ...
        text = cast(lexbor_str_t*) lexbor_array_obj_push(pt_list);
        if (text == null) {
    ...
    if (tree.pending_table.have_non_ws) {
        ...
        for (size_t i = 0; i < lexbor_array_obj_length(pt_list); i++) {
            text = cast(lexbor_str_t*) lexbor_array_obj_get(pt_list, i);
...
private void lxb_html_tree_insertion_mode_in_table_text_erase(lxb_html_tree_t* tree)
{
    lexbor_str_t* text = void;
    lexbor_array_obj_t* pt_list = tree.pending_table.text_list;
