module parserino.lexbor.html.tree.insertion_mode.in_head;
...
import parserino.lexbor.html.interfaces.script_element;
...
private bool lxb_html_tree_insertion_mode_in_head_open(lxb_html_tree_t* tree, lxb_html_token_t* token)
{
    switch (token.tag_id) {
...
private bool lxb_html_tree_insertion_mode_in_head_closed(lxb_html_tree_t* tree, lxb_html_token_t* token)
{
    switch (token.tag_id) {
        case LXB_TAG_HEAD:
            lxb_html_tree_open_elements_pop(tree);
 ...
 bool lxb_html_tree_insertion_mode_in_head_script(lxb_html_tree_t* tree, lxb_html_token_t* token)
{
    lxb_dom_node_t* ap_node = void;
    lxb_html_element_t* element = void;
    ...
    if (tree.status != LXB_HTML_STATUS_OK) {
        lxb_html_script_element_interface_destroy((cast(lxb_html_script_element_t*) (element)));

        return lxb_html_tree_process_abort(tree);
    }

    lxb_html_tree_insert_node(ap_node, (cast(lxb_dom_node_t*) (element)), ipos);
 ...
 bool lxb_html_tree_insertion_mode_in_head_template(lxb_html_tree_t* tree, lxb_html_token_t* token)
{
    lxb_html_element_t* element = void;
    ...
    if (tree.status != LXB_STATUS_OK) {
        lxb_html_template_element_interface_destroy((cast(lxb_html_template_element_t*) (element)));
 ...
 bool lxb_html_tree_insertion_mode_in_head_template_closed(lxb_html_tree_t* tree, lxb_html_token_t* token)
{
 ...
 bool lxb_html_tree_insertion_mode_in_head_anything_else(lxb_html_tree_t* tree)
{
    lxb_html_tree_open_elements_pop(tree);
